...
public class Texture {
    ...
    public Texture(String fileName) {
        this(fileName, GL_TEXTURE_2D, GL_LINEAR, GL_RGBA8, GL_RGBA, false, GL_COLOR_ATTACHMENT0);
    }
    ...
    public Texture(String fileName, int textureTarget, int filter, int internalFormat, int format, boolean clamp, int attachment) {
        ...
        if (oldResource != null) {
            ...
            resource = loadTexture(fileName, textureTarget, new int[]{filter}, new int[]{internalFormat}, new int[]{format}, clamp, attachment == 0 ? new int[]{attachment} : null);
    ...
    public Texture(int width, int height, ByteBuffer data, int textureTarget, int filter, int internalFormat, int format, boolean clamp, int attachment) {
        resource = new TextureResource(textureTarget, width, height, 1, data, new int[]{filter}, new int[]{internalFormat}, new int[]{format}, clamp, attachment != 0 ? new int[]{attachment} : null);
    }
